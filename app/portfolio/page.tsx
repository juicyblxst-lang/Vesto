...
export default function Portfolio() {
  const t = theses[0]; const { address } = useAccount();
  const [allocations, setAllocations] = useState<Record<string, number>>(Object.fromEntries(t.assets));
  const [notice, setNotice] = useState('');
  const total = Object.values(allocations).reduce((a,b)=>a+b,0);
  const change = (symbol: string, value: string) => setAllocations(a => ({...a, [symbol]: Math.max(0, Math.min(100, Number(value) || 0))}));
  const save = async () => {
    ...
    const d=await r.json(); setNotice(r.ok ? 'Remix saved to Vesto.' : d.error || 'Unable to save.');
  };
